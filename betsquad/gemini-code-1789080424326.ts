...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    // 1. Carica quote tramite Proxy API
    fetch('/api/odds')
      .then((res) => res.json())
      .then((d) => setMatches(d.matches || []))
      .finally(() => setLoading(false));

    // 2. Carica pronostici iniziali della stanza
    supabase
      .from('room_picks')
      .select('*')
      .eq('room_id', roomId)
    ...
    // 3. Iscrizione Real-Time Supabase WebSocket
  ...
  // Anti-Correlazione: 1 selezione per partita
  const proposePick = async (match: any, market: string, selection: string, odds: number) => {
    ...
    if (exists) {
      alert('⚠️ Questa partita è già in schedina! Rimuovila per cambiarla.');
      return;
    }

    await supabase.from('room_picks').insert({
      room_id: roomId,
      match_id: match.id,
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {Object.entries(m.odds1X2).map(([lbl, val]: any) => (
                      ...
                      >
                        ...
                        <span className="text-xs font-bold text-[#00e676]">@{val.toFixed(2)}</span>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-800/60">
                    ...
                    <div className="flex gap-2 overflow-x-auto">
                      {Object.entries(m.derived.combo).slice(0, 3).map(([cName, cVal]: any) => (
                        <button
                          key={cName}
                          onClick={() => proposePick(m, 'Combo', cName, cVal)}
