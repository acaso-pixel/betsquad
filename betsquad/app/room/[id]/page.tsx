...
export default function RoomPage() {
  ...
  // Area 3: Ricerca e preferiti campionati
  ...
  // Dettaglio Partita 15 Categorie
  ...
  // AREA 1: Gestione votazione e chiusura automatica a maggioranza (>50%)
  ...
  const votePick = async (pick: any, val: number) => {
    ...
    // Regola Maggioranza Semplice (> 50% dei presenti)
  ...
  // AREA 2: Array estendibile di bookmaker con logica di compatibilità mercati
  ...
  const evaluatedBookmakers = useMemo(() => {
    return bookmakersConfig.map((b) => {
      // Verifica se il bookmaker supporta tutti i mercati presenti in schedina
  ...
  // AREA 3: Condivisione nativa Web Share API + QR Code URL
  const shareRoomLink = () => {
    const shareData = {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)] text-[var(--text-main)] pb-44 font-sans antialiased select-none">
        ...
        >
          ...
          >
            ...
            <div className="bg-[var(--surface-sub)] p-3 rounded-lg border border-[var(--border-subtle)] flex items-center justify-center">
              {/* QR Code generato via API pubblica con URL stanza */}
              <img 
      ...
      {/* Tabs Principali (Include la nuova Tab Storico Votazioni - Area 1) */}
      ...
      <main className="p-2 sm:p-4 w-full">
        {activeTab === "palinsesto" && (
          <div className="flex flex-col md:flex-row gap-3 items-start w-full">
            {/* Sidebar Desktop con Ricerca e Preferiti Fissati (Area 3) */}
            ...
            <div className="flex-1 min-w-0 w-full space-y-3">
              ...
              {/* AREA 3: SKELETON LOADING al posto del testo statico */}
              {loading ? (
        ...
        {/* Tab 1 (AREA 1): VOTAZIONI LIVE CON BARRA DI PROGRESSO E MAGGIORANZA */}
        {activeTab === "voti" && (
        ...
        {/* Tab Storico Votazioni Concluse (Area 1) */}
        {activeTab === "storico" && (
        ...
        {/* Tab 3: Schedina Squad & AREA 2: Confronto Multi-Bookmaker Intelligente */}
        {activeTab === "schedina" && (
            ...
            {confirmed.length === 0 ? (
              ...
              <div className="space-y-4">
                ...
                <div className="pt-3 border-t border-[var(--border-subtle)] space-y-2 text-xs">
                  ...
                  {/* AREA 2: CONFRONTO MULTI-BOOKMAKER ORDINATO PER PAYOUT E VERIFICA DISPONIBILITÀ */}
